<template>
  ...
  export default {
    ...
    async mounted() { 
      ...
      this.csv = this.dataNode.data.csv.path;
    },
    methods: {
      loadExcelFile(event){
  this.el=this.$refs.el;
  ...
  const filePath = `C:/uploads/${fileName}`; // Concatenate upload folder path with file name
  this.csv = filePath;
  this.nodeId = this.el?.parentElement.parentElement.id.slice(5)
  this.dataNode = this.df.getNodeFromId(this.nodeId)
  ...
  reader.onload = (e) => {
    ...
    const outputFile = path.join('C:\\uploads\\', `${fileName}`);
